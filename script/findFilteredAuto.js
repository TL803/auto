...
form.addEventListener('submit', function (e) {
    ...
    const filtered = carDataArray.filter(car => {
        return (
            ...
            (driveType === '' || car.drive_type.includes(driveType)) && 
            (bodyType === '' || car.body_type === bodyType) &&
            car.priceValue >= priceFrom &&
            car.priceValue <= priceTo
        );
    });
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<p class="text-gray-600 text-center py-6">По вашему запросу ничего не найдено.</p>';
        return;
    }
