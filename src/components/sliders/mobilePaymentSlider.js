...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function generateCards() {
        ...
        terms.forEach((item, index) => {
            ...
            card.setAttribute("data-year", String(8 - index));
            card.setAttribute("data-term", item.term);
            card.setAttribute("data-payment", item.payment);
            card.setAttribute("data-index", String(index));

            card.style.cssText =
                "box-shadow: 0 20px 50px rgba(0, 0, 0, 0.1); border-radius: 0.5rem;";
            ...
            card.innerHTML = `
              ...
              <span class="text-[46px] font-medium mt-2 paymentValue">${item.payment}</span>
    ...
    function updateVisibleCards() {
        visibleCards = cards.slice(currentIndex * 6, (currentIndex + 1) * 6);
        resetCards();
