document.addEventListener("DOMContentLoaded", function () {
    ...
    creditModal.className =
        "hidden w-full h-full bg-black/50 fixed z-[10] inset-0 flex items-center justify-center";

    // Содержимое модального окна
    const modalContent = document.createElement("div");
    modalContent.className =
        "bg-white rounded-[14px] w-[1200px] shadow-lg p-12 h-[600px] flex flex-col justify-center relative";

    // Кнопка закрытия
    const closeBtn = document.createElement("button");
    closeBtn.id = "closeCreditModal";
    closeBtn.className =
        "absolute top-4 right-4 text-gray-600 hover:text-gray-800 focus:outline-none";
    closeBtn.innerHTML = `
    ...
    nameInput.className =
        "w-full px-7 py-6 bg-[#F8F8F8] rounded-md focus:outline-none";
    ...
    label.innerHTML = `
          я согласен с <a class="text-blue-600 underline" href="/privacypolicy">политикой обработки персональных данных</a>
      `;
    ...
    submitBtn.className =
        "w-full bg-[#4886FF] text-white px-4 py-3 rounded md:text-xl font-medium";
